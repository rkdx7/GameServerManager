...
QWidget *GenericGamePage::buildInstallForm()
{
    ...
    // ── Header row: icon+title on left, gear button on right ─────────────
    auto *iconLbl  = new QLabel(m_config.icon, container);
    iconLbl->setStyleSheet("font-size: 40px; background: transparent;");
    auto *titleLbl = new QLabel(m_config.title, container);
    titleLbl->setStyleSheet("font-size: 24px; font-weight: 700; color: #1e293b; background: transparent;");
    auto *descLbl  = new QLabel(m_config.description, container);
    descLbl->setStyleSheet("font-size: 13px; color: #64748b; background: transparent;");
